...
def obstaclecheck(x, y, radius, clearance):
    tolerance = radius + clearance

    obstacle1 = ((np.square(x - 2)) + (np.square(y - 2)) <= np.square(1 + tolerance))
    obstacle2 = ((np.square(x - 2)) + (np.square(y - 8)) <= np.square(1 + tolerance))
    obstacle3 = (x >= 3.75 - tolerance) and (x <= 6.25 + tolerance) and (y >= 4.25 - tolerance) and (y <= 5.75 + tolerance)
    obstacle4 = (x >= 7.25 - tolerance) and (x <= 8.75 + tolerance) and (y >= 2 - tolerance) and (y <= 4 + tolerance)
    obstacle5 = (x >= 0.25 - tolerance) and (x <= 1.75 + tolerance) and (y >= 4.25 - tolerance) and (y <= 5.75 + tolerance)
    
    border1 = (x <= 0.0 + tolerance)
    border2 = (x >= 9.99 - tolerance)
    border3 = (y <= 0.01 + tolerance)
    border4 = (y >= 9.99 - tolerance)
